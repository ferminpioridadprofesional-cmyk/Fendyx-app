...
async function checkExistingSession() {
    try {
        ...
        if (session) {
            ...
            // Aquí redirigiremos al dashboard cuando lo creemos
            showMessage('¡Bienvenido de nuevo, ' + session.user.email + '!', 'success');
        }
    } catch (error) {
...
async function handleLogin(event) {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            console.log('Redirección al dashboard (pendiente)');
        }, 1500);
...
async function handleRegister(event) {
    ...
    try {
        ...
        if (data.user && !data.session) {
            showMessage('¡Cuenta creada! Revisa tu correo para confirmar tu cuenta.', 'success');
        } else {
            showMessage('¡Cuenta creada exitosamente! Bienvenido a Fendyx 🎉', 'success');
        }
...
document.addEventListener('DOMContentLoaded', loadAppLogo);
